...
const TOKEN_URL = process.env.ADLOOK_OAUTH_TOKEN_URL?.trim() ||
...
let cachedAccessToken = process.env.ADLOOK_TOKEN?.trim() || null;
...
export function isOAuthRefreshConfigured() {
 ...
 */
export function setSessionAuth(accessToken, refreshToken) {
...
async function postRefreshJson() {
    ...
    if (!res.ok) {
        throw new Error(`Odświeżenie tokenu nie powiodło się: HTTP ${res.status}: ${JSON.stringify(data)}`);
    }
    const access = pickAccessToken(data);
    if (!access) {
        throw new Error(`Odświeżenie tokenu: brak access tokena w odpowiedzi (accessToken / access_token / token): ${JSON.stringify(data)}`);
    }
...
async function postRefreshOAuth2Form() {
    ...
    const data = (await res.json());
    if (!res.ok) {
        throw new Error(`Odświeżenie tokenu OAuth nie powiodło się: HTTP ${res.status}: ${JSON.stringify(data)}`);
    }
    const access = pickAccessToken(data);
    if (!access) {
        throw new Error(`Odświeżenie tokenu: brak access tokena w odpowiedzi: ${JSON.stringify(data)}`);
    }
    cachedAccessToken = access;
    const nextRefresh = pickRefreshToken(data);
    if (nextRefresh) {
 ...
 */
export async function resolveAccessToken(tokenOverride) {
    ...
    if (!isOAuthRefreshConfigured()) {
        ...
        if (!t) {
            throw new Error("Brak tokenu autoryzacyjnego. Na początku sesji wywołaj set_adlook_auth (token z przeglądarki), " +
    ...
    if (!cachedAccessToken) {
        throw new Error(`Brak access tokenu po odświeżeniu. Sprawdź refresh_token (set_adlook_auth lub ADLOOK_REFRESH_TOKEN) i URL: ${TOKEN_URL}`);
    }
